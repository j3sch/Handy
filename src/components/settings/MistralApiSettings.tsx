...
interface MistralApiSettingsProps {
  onApiKeySet?: () => void;
}

const MistralApiSettings: React.FC<MistralApiSettingsProps> = ({ onApiKeySet }) => {
  ...
  const loadApiKeyStatus = async () => {
    try {
      const hasKey = await invoke<boolean>("has_mistral_api_key");
      setHasApiKey(hasKey);
      if (hasKey && showApiKey) {
        const key = await invoke<string | null>("get_mistral_api_key");
        setApiKey(key || "");
      }
    } catch (error) {
  ...
  const handleSaveApiKey = async () => {
    setLoading(true);
    try {
      await invoke("set_mistral_api_key", { apiKey });
...
export default MistralApiSettings;
